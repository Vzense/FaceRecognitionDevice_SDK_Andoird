			...
			public void onItemSelected(AdapterView<?> parent, View view,
				...
				if (str.equals("FaceMode")) {
					if(bHasDeviceRegister && null != mVzenseCamera) {
						ret = mVzenseCamera.setWorkMode(PsCamera.FACE_MODE);
						if (ret == 0) {
							mWorkMode = 1;
					...
					if(bHasDeviceRegister && null != mVzenseCamera) {
						ret = mVzenseCamera.setWorkMode(PsCamera.CODE_MODE);
						if (ret == 0) {
							mWorkMode = 2;
...
//							Toast.makeText(MainActivity.this, "Upgrade failed. Others errors.", Toast.LENGTH_SHORT).show();
		...
		if (mVzenseCamera != null) {
			mVzenseCamera.init(this, mOnVzenseCameraConnectListener);
		}
	...
	private final OnVzenseCameraConnectListener mOnVzenseCameraConnectListener = new OnVzenseCameraConnectListener() {
		@Override
